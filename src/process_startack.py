...
COLUMN NAMES:
=============
    ['mdot', 'm', 'Z', 'tage', 'is_bh', 'mdot_gs', 'LEdd', 'mdot_Edd',
...
def main():
    ...
    df_master['mdot_gs'] = mdot * (M_sol/Myr)   
    ...
    df_master['mdot_Edd'] = LEdd / (1/12 * c**2)
    
    
    # Eddington Ratio
    df_master['mdot_ratio'] = mdot_gs / df_master['mdot_Edd'] #AKA Eddington Ratio
    ...
    df_master['theta_deg'] = df_master['theta'] * 180/np.pi  # degrees
    df_master['theta_half_deg'] = df_master['theta_deg'] / 2 # Half opening angle (theta / 2)
    
    
    df_master['zeta'] = np.tan((np.pi/2) - np.arccos(1 - (73/(df_master['mdot_ratio']**2))))
    ...
    df_master['P_inflow_at_rsph'] = (G * m * M_sol * np.pi) / (3 * c**3 * df_master['a*']) * df_master['r_sph']**3 * ((1 - (df_master['r_isco']/df_master['r_sph']))**3)/(np.log(df_master['r_sph']/df_master['r_isco']))
    df_master['P_envelope']       = (G * m * M_sol * np.pi) / (3 * c**3 * df_master['a*']) * df_master['r_sph']**3 * ((1 - (df_master['r_isco']/df_master['r_sph']))**3)/(np.log(df_master['r_sph']/df_master['r_isco'])) * (df_master['r_out']/df_master['r_sph'])**2
    df_master['P_wind']           = (G * m * M_sol * np.pi) / (3 * c**3 * df_master['a*']) * df_master['r_out']**3 * ((1 - (df_master['r_isco']/df_master['r_out']))**3)/(np.log(df_master['r_out']/df_master['r_isco']))
