...
int main(int argc, char *argv[]){
	...
	sql = sqlite3_mprintf("SELECT system_row_id, theta_half_deg, Lx1, P_wind_days, P_sup_days, inclination, dincl from XLF_MC_SAMPLED_SYSTEMS WHERE run_id=%Q", run_id);
	...
	while(sqlite3_step(res) == SQLITE_ROW){
		...
		system_inclination[r] = sqlite3_column_int(res, 3);
		system_dincl[r]	      = sqlite3_column_int(res, 4);
		r++;
	}
