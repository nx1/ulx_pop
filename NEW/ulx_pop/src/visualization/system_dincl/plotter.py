...
for i in df_dict:
    ...
    inclination = split[2]
    Lx = GetLx(int(simNum))
    print('{:10} {:10} {:10}'.format('simnum', 'dincl', 'inclination'))
    print('{:10} {:10} {:10}'.format(simNum, dincl, inclination))
    
    curves = FindCurves(df_dict, simNum, dincl)
    for key in curves:
...
anim_dict = {}
for i in df_dict:
    anim_dict[i.split('-')[1]] = df_dict[i]['Time'],df_dict[i]['Flux']
...
def init():
    ...
    '''
    line.set_ydata([np.nan]*len(anim_dict['0'][0]))
    text.set_text('')
    return line, 

def animate(i):
    strN = str(i)
    line.set_xdata(anim_dict[strN][0])
    line.set_ydata(anim_dict[strN][1])
    return line,  text

ani = animation.FuncAnimation(fig, animate, init_func = init, interval = 500, blit=True, repeat=True, frames=50)
